...
# Функция для негативной проверки пустого параметра Имя
# В ответе ошибка: "Не все необходимые параметры были переданы"
def negative_assert_no_firstname(user_body):
# В переменную user_body сохраняется обновленное тело запроса
   # user_body = get_user_body(first_name) ЭТО НЕ НАДО ПОТОМУ ЧТО ТЕЛО user_body получает функция на вход
...
# Проверка, что в теле ответа атрибут "code" равен 400
    assert response.json()["code"]  == 400
# Проверка, что в ответе есть текст сообщения об ошибке
...
def test_create_user_empty_first_name_get_error_response():
    ...
    negative_assert_no_firstname(user_body)
# Тест 12. Ошибка
# Параметр firstName состоит из чисел
